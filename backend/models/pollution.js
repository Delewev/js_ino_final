...
class Pollution {
    ...
    static async createItem({address, lan, el, item, aqi, components}) {
        const insertRes = await pool.query('insert into pollutions(created_at, address, lat, lon, measured_at, aqi, components) values($1, $2, $3, $4, $5, $6, $7) RETURNING *', [
            'now()', address, lan, el, dayjs(item).format('YYYY-MM-DD HH:mm:ss'), aqi, components,
        ]);
        return insertRes.rows[0]
    }
}

module.exports = {Pollutions: Pollution}
